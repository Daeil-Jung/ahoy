...
3. If `attempt < max_attempts` (default 3):
   - status → `rework`
   - When calling /ahoy:ahoy-gen for rework with attempt >= 2:
     1. Read previous issues.json.attempt-N files from the sprint directory
     2. Include avoidance summary in the sub-agent prompt context
     3. The avoidance summary can be generated by reading gen_report.md and issues.json.attempt-{N} files
