...
AHOY는 범용 에이전트 프레임워크가 아니라, AI가 생성한 코드를 검증하는 **외부 리뷰 게이트**입니다.
...
```bash
# 현재 git diff를 가볍게 리뷰한다. 스프린트 상태는 만들지 않는다.
/ahoy:review-diff

# strict 모드: 설정된 evaluator quorum이 없으면 실패한다.
...
### review-diff와 전체 `/ahoy`의 차이

`/ahoy:review-diff`는 이미 로컬 변경이 있고 `git diff HEAD --`만 외부 모델로 빠르게 검토하고 싶을 때 사용한다. `.claude/harness/sprints/...` 상태를 만들거나 수정하지 않는다. advisory 모드는 evaluator 1개로도 피드백을 제공하고, strict 모드는 설정된 quorum이 없으면 fail-closed로 종료한다.

전체 `/ahoy`는 계획, 스프린트 계약, Generator/Evaluator 분리, 영속 `issues.json`, rework loop가 필요할 때 사용한다.
...
| 스킬 | 역할 | 설명 |
|------|------|------|
| **`/ahoy`** | **Orchestrator** | **전체 스프린트 라이프사이클 관리** |
| `/ahoy:review-diff` | Lightweight reviewer | 스프린트 상태 없이 현재 git diff 리뷰 |
